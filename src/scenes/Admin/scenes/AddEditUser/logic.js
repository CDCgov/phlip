...
export const addUserLogic = createLogic({
  ...
  async process({ action, api }, dispatch, done) {
    try {
      ...
      dispatch({ type: types.ADD_USER_FAIL, payload: 'We failed to add this user. Please try again later.' })
    }
    done()
  }
})
      ...
      if (e.response.status === 304) {
        ...
        dispatch({
          ...
          payload: action.selfUpdate
            ? 'We failed to update your profile. Please try again later.'
            : 'We failed to update this user. Please try again later.'
        })
      }
    }
    done()
  ...
  async process({ action, api }, dispatch, done) {
    try {
      ...
      dispatch({
        ...
        payload: action.selfUpdate
          ? 'We failed to add your photo. Please try again later.'
          : 'We failed to add a photo for this user. Please try again later.'
      })
    }
    done()
  }
  ...
  async process({ action, api }, dispatch, done) {
    try {
      ...
      dispatch({
        ...
        payload: action.selfUpdate
          ? 'We failed to remove your photo. Please try again later.'
          : 'We failed to remove the photo for this user. Please try again later.'
      })
    }
    done()
  }
